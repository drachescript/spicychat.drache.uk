...
- rounded current totals and exact totals are stored separately so an approximate update no longer erases the last exact count
- the updater can recover the last exact count from saved history after an older rounded update already replaced the current row
- exact values are only allowed to replace a rounded value when they fit that rounded value's real rounding range, avoiding stale exact-count regressions

## v0.7.2 — September 12, 2026

- exact Typesense message totals now take priority over rounded creator-page values when both are available
- the main Stats overview keeps message counts compact, so an exact 3,205 is shown as approximately 3.2k instead of a long raw number
- bot detail pages now show both the compact message total and the exact stored message count
